      ...
      <div className="p-4 max-w-md mx-auto space-y-6">
        ...
        <section className="bg-white p-4 rounded-xl shadow-sm space-y-4 border border-gray-200">
          ...
          <div className="grid grid-cols-1 gap-3">
            <div><label className="block text-xs text-gray-500 uppercase">Nome Principal</label><p className="text-gray-900 font-medium">{reserva.nome_hosp_princ}</p></div>
            <div><label className="block text-xs text-gray-500 uppercase">Telefone</label><p className="text-gray-900 font-medium">{reserva.telefone}</p></div>
            <div><label className="block text-xs text-gray-500 uppercase">Acomodação</label><p className="text-gray-900 font-medium">🏠 {reserva.acomodacao}</p></div>
          </div>
        </section>
        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => router.push(`/editar/${id}`)} className="bg-gray-200 text-gray-800 font-bold py-3 rounded-xl hover:bg-gray-300 transition-all">Editar Reserva</button>
          <button onClick={() => router.push('/')} className="bg-blue-600 text-white font-bold py-3 rounded-xl hover:bg-blue-700 transition-all">Voltar Home</button>
        </div>
      </div>
    </main>
  );
